...
def class_decorator(task_name):
    envs_module = importlib.import_module(f'envs.{task_name}') #block_hammer_beat.py
    try:
        env_class = getattr(envs_module, task_name) #class envs/block_hammer_beat
        env_instance = env_class()
    except ModuleNotFoundError:
        raise SystemExit("No such task")
    return env_instance  #envs/block_hammer_beat
...
def run(Demo_class, args):
    ...
    if args['collect_data']:
        ...
        for id in range(args['st_episode'], args['episode_num']): #从起始Episode st_episode 到总Episode数 episode_num
            Demo_class.setup_demo(now_ep_num=id, seed = seed_list[id],**args)
            # modify start 保存为pkl文件
            ...
            if Demo_class.save_type.get('raw_data', True): #保存相机配置
